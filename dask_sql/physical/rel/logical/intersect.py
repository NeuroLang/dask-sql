...
class LogicalIntersectPlugin(BaseRelPlugin):
    """
    LogicalIntersect is used on INTERSECT clauses.
    It just concatonates the two data frames.
    """

    class_name = "org.apache.calcite.rel.logical.LogicalIntersect"
    ...
    def convert(
        ...
        first_cc = first_cc.rename(
            columns={
                col: output_col
                for col, output_col in zip(
                    first_cc.columns, output_field_names
                )
            }
        )
        ...
        second_cc = second_cc.rename(
            columns={
                col: output_col
                for col, output_col in zip(
                    second_cc.columns, output_field_names
                )
            }
        )
        ...
        self.check_columns_from_row_type(
            first_df, rel.getExpectedInputRowType(0)
        )
        self.check_columns_from_row_type(
            second_df, rel.getExpectedInputRowType(1)
        )

        df = first_df.merge(second_df, how="inner")
